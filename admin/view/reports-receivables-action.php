...
if ( isset( $_GET ) ) {
  ...
  if ( $_GET['action'] == "d" ) {
		...
		$query = "SELECT `paid_date` AS letter, `total` AS frequency FROM `invoices_receivable` WHERE status='paid' ORDER BY `paid_date` ASC";
		$result = mysql_query( $query );
		$array_invoices = array();
		while( $row = mysql_fetch_array( $result )) {
			$array_invoices[] = array( "letter"=>$row[letter], "frequency"=>$row[frequency] );
		}
		file_put_contents( "_invoice_receivable_daily.tsv", "letter\tfrequency\n", FILE_APPEND | LOCK_EX );
		foreach( $array_invoices as $arr => $value ) {
			file_put_contents( "_invoice_receivable_daily.tsv", $value[letter] . "\t" . $value[frequency] . "\n", FILE_APPEND | LOCK_EX  );
		}
		rename("_invoice_receivable_daily.tsv", "invoice_receivable_daily.tsv");
		...
		file_put_contents( "_invoice_receivable_monthly.tsv", "letter\tfrequency\n", FILE_APPEND | LOCK_EX );
		foreach( $array_invoices as $arr => $value ) {
			file_put_contents( "_invoice_receivable_monthly.tsv", $value[letter] . "\t" . $value[frequency] . "\n", FILE_APPEND | LOCK_EX  );
		}
		rename("_invoice_receivable_monthly.tsv", "invoice_receivable_monthly.tsv");
		...
		file_put_contents( "_invoice_receivable_yearly.tsv", "letter\tfrequency\n", FILE_APPEND | LOCK_EX );
		foreach( $array_invoices as $arr => $value ) {
			file_put_contents( "_invoice_receivable_yearly.tsv", $value[letter] . "\t" . $value[frequency] . "\n", FILE_APPEND | LOCK_EX  );
		}
		rename("_invoice_receivable_yearly.tsv", "invoice_receivable_yearly.tsv");
		...
		// query by month (due_date) but with all status 
		...
		file_put_contents( "_invoice_receivable_monthly_all.tsv", "letter\tfrequency\n", FILE_APPEND | LOCK_EX );
		foreach( $array_invoices as $arr => $value ) {
			file_put_contents( "_invoice_receivable_monthly_all.tsv", $value[letter] . "\t" . $value[frequency] . "\n", FILE_APPEND | LOCK_EX  );
		}
		rename("_invoice_receivable_monthly_all.tsv", "invoice_receivable_monthly_all.tsv");
...
-->
    ...
    var tip = d3.tip()
      ...
      .offset([-10, 0])
      .html(function(d) {
	return "<strong>Entradas:</strong> <span style='color:red'>R$ " + d.frequency + "</span> <br> <strong>Data:</strong> <span style='color:red'> " + d.letter + "</span>";
      })
