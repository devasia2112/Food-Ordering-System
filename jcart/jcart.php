	...
	*/
	public function display_cart() {
		...
		if ($errorMessage)	{
		    ...
		    echo tab(3) . "<table border='0' width='100%'>\n";
		    echo tab(4) . "<thead>\n";
		    echo tab(5) . "<tr>\n";
		    echo tab(6) . "<th colspan='3' align='left'>\n";
		    echo tab(7) . "<h2><img src='images/icons/check-alt.png' />  ".$config['text']['cartTitle']." ($this->itemCount $itemsText) </h2>\n";
		    echo tab(6) . "</th>\n";
		    echo tab(5) . "</tr>". "\n";
		    echo tab(4) . "</thead>\n";
		    ...
		    if ($isCheckout !== true) {
			    ...
			    // If delivery area zipcode match then allow delivery to that area otherwise deny
			    if ( isset( $_SESSION['allow_checkout'] ) and $_SESSION['allow_checkout'] == 1 )
			    {
			      echo tab(7) . "<input type='$inputType' $src id='jcart-checkout' name='jcartCheckout' class='jcart-button' value='{$config['text']['checkout']}' />\n";
			    }
		    }

		    //echo tab(5) . "<tr><td colspan='3'><div align=right>Entrega: <b>$currencySymbol" . number_format($_SESSION['valor_entrega'], $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "</b></div></td></tr>\n";

		    // If order from PCS personal chef service then valor_entrega is ZERO else add value
		    if ( !empty( $_SESSION['PCS']['order_id'] )) { $valor_entrega = 0; } else { $valor_entrega = $_SESSION['valor_entrega']; }

		    echo tab(7) . "<span id='jcart-subtotal'>
				      <div align=left>".$config['text']['chargeDeliveryLabel']." <b>$currencySymbol" . number_format($valor_entrega, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "</b></div>
				      {$config['text']['subtotal']}: <strong>$currencySymbol" . number_format($this->subtotal+$valor_entrega, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "</strong>
				  </span>\n";


			// <!-- validate AGENDAMENTO ---  o agendamento esta sendo feito na pagina do menu apenas , se o cliente quiser trocar a data do agendamento, entao deve clicar em "Continue Pedindo" em checkout para isso ... -->
			// echo tab(7) . '<br><div align=left><b>Agendamento</b></div><iframe src="scheduler.php" width="325" height="275" scrolling="no" frameborder=0></iframe>';
		    ...
		    // The cart is empty
			    ...
			    echo tab(5) . "<tr><td id='jcart-empty' colspan='3'>".$config['text']['chargeDeliveryLabel']." <b>$currencySymbol" . number_format($_SESSION['valor_entrega'], $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "</b></td></tr>\n";
		    }
		...
		// If this is the checkout display the Payment (PayPal) checkout button
		if ($isCheckout === true) {
...
////                <!-- Step 2 - Field Observation about the order -->
		    echo '<br /><br />
			  <h2><img src="images/icons/check-alt.png" />  ' . LBL_OBSERVATION_ORDER . ' </h2>
			  <div><i>' . LBL_OBSERVATION_ORDER_DETAILS . '</i></div>
			  <hr />
			  <div><br />
			      <textarea name="observation_order" cols="60" rows="3"></textarea>
			  </div><br />';
			echo "<i> ".LBL_ORDER_SCHEDULED." <b>" . $_SESSION['agenda_checkout'] . "</b></i><br>". LBL_ORDER_SCHEDULED_CHANGE."<br><br>";
...
////                <!-- Step 3 - Delivery -->
			...
			echo '<br />
				    <h2><img src="images/icons/check-alt.png" />  ' . LBL_DELIVERY_ORDER . ' </h2>
				    <hr />';

			// In case of user is logged using facebook we need to check the address otherwhise the user is logged with default log in system
			if (isset($_SESSION['USER_EMAIL']))
			{
			    ...
			    {
				// Check Address
				if (( $customers_data['street'] == "" ) or empty( $customers_data['street'] ))
				{
				    echo '<pre>Não foi encontrado um endereço para entrega.<br />';
				    echo '<a href="customer-registration?endereco=atualizar">'.LBL_UPDATE_ADDRESS.'</a>';
				    echo '</pre>';
				...
				else
				{
				    ...
				    // Create a session ID customer to deal in the next step
				    $_SESSION['IDCUSTOMER'] = $customer_id;
				}
			    }
			}
			else if ( isset( $_SESSION['IDCUSTOMER'] ) and !empty( $_SESSION['IDCUSTOMER'] ))
			{
			    $array_customer = GenericSql::getCustomerById( $_SESSION['IDCUSTOMER'] );

			    // Check Address
			    if (( $array_customer['street'] == "" ) or empty( $array_customer['street'] ))
			    {
				echo '<pre>Não foi encontrado um endereço para entrega.<br />';
				echo '<a href="customer-registration?endereco=atualizar">'.LBL_UPDATE_ADDRESS.'</a>';
				echo '</pre>';
			    ...
			    {
				echo '<table>';
				echo '<tr><td width=180> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_STREET.':</b> </td><td> '.$array_customer['street'].' </td></tr>';
				echo '<tr><td> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_NUMBER.':</b> </td><td> '.$array_customer['number'].' </td></tr>';
				echo '<tr><td> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_MISC.':</b> </td><td> '.$array_customer['complement'].' </td></tr>';
				echo '<tr><td> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_SUBURB.':</b> </td><td> '.$array_customer['suburb'].' </td></tr>';
				echo '<tr><td> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_STATE.':</b> </td><td> '.$array_customer['state'].' </td></tr>';
				echo '<tr><td> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_TOWN.':</b> </td><td> '.$array_customer['town'].' </td></tr>';
				echo '<tr><td> <b>&nbsp;&nbsp;'.LBL_CUSTOMER_ZIPCODE.':</b> </td><td> '.$array_customer['zipcode'].' </td></tr>';
				echo '</table>';
				echo '
				    <div style="width: 19%; padding: 1px; float: right;">
					<input type="submit" name="update-address" value="&larr; '.LBL_UPDATE_ADDRESS.'" id="jcart-paypal-checkout" />
				    </div><br /><br /><br />';
			    }
			...
			{
			    echo '<pre>ELSE NULL :: NEED LOG IN</pre>';
			    //In case of customer do not have address yet -- need to query to know the answer
				    $disablePaypalCheckout = " disabled='disabled'";
			}
...
////                <!-- Step 4 - Payment Method -->
			...
			echo '<br /><br />
				...
				<div style="width: 100%;">
				  <div style="width: 19%; padding: 1px; float: left;">
				      <input type="submit" name="continueorder" value="&larr; '.LBL_ORDER_MORE.'" id="jcart-paypal-checkout" />
				  </div>
				  <div style="width: 19%; padding: 1px; float: left;"> &nbsp;&nbsp;&nbsp; </div>
				  <div style="width: 19%; padding: 1px; float: left;">
